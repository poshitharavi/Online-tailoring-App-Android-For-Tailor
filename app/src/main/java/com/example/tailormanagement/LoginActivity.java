        ...
        loginBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (emailTxt.getText().toString().length() > 0 && passwordTxt.getText().toString().length() > 0) {
                    ...
                    JsonObjectRequest req = new JsonObjectRequest(Request.Method.POST, getResources().getString(R.string.api_url) + "tailor/auth-tailor", reqObj, new Response.Listener<JSONObject>() {
                        @Override
                        public void onResponse(JSONObject jsonObject) {
                            Log.d("Data: ", jsonObject.toString());
                            try {
                                if (!jsonObject.has("status")) {
                                    SharedPreferences sharedPreferences = getSharedPreferences("tailorManagementDetails", Context.MODE_PRIVATE);
                                    SharedPreferences.Editor editor = sharedPreferences.edit();

                                    editor.putString("tailor_id", jsonObject.getString("tailorId"));
                                    editor.putString("tailor_user_name", jsonObject.getString("name"));
                                    editor.putString("tailor_email", jsonObject.getString("email"));
                                    editor.commit();
